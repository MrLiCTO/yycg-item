...
<script type="text/javascript">
	...
	
	{
		...
		width : 120,
		formatter : function(value, row, index) {//通过此方法格式化显示内容,value表示从json中取出该单元格的值，row表示这一行的数据，是一个对象,index:行的序号
			return "<a href=javascript:deletesysuser('"+row.id+"')>删除</a>&nbsp;&nbsp;&nbsp;<a href=javascript:editsysuser('"+row.id+"')>编辑</a>"
		}
	} 
	...
	function editsysuser(id){
		//打开一个窗口，用户添加页面
		//参数：窗口的title、宽、高、url地址
		createmodalwindow("添加用户信息", 800, 250, '${baseurl}user/editsysuser.action?id='+id);
	}
</script>
		...
		<TBODY>
			<TR>
				...
				<td><select name="sysuserCustom.groupid">
						...
						<c:forEach items="${groupList}" var="gmc">
						<option value="${gmc.dictcode}">${gmc.info}</option>
